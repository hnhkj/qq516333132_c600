	...
	if (ep->dma_working) {
		if (g_dma_debug) {
			...
			DMSG_PANIC("ERR: dma is busy, write fifo. ep(0x%p, %d), req(0x%p, 0x%p, 0x%p, %d, %d)\n\n",
				ep, ep->num,
				req, &(req->req), req->req.buf, req->req.length, req->req.actual);
			...
			if (req_next) {
				DMSG_PANIC("ERR: dma is busy, write fifo. req(0x%p, 0x%p, 0x%p, %d, %d)\n\n",
					req_next, &(req_next->req), req_next->req.buf, req_next->req.length, req_next->req.actual);
			}
		}
...
static inline int sunxi_udc_read_packet(void __iomem *fifo, void *buf,
...
static int sunxi_udc_get_status(struct sunxi_udc *dev, struct usb_ctrlrequest *crq)
{
	...
	udelay(5);
	USBC_Dev_ReadDataStatus(g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_EP0, 0);/* toggle to 0 */
						...
						u32 ep0csr)
{
	...
	if (len != sizeof(*crq)) {
		//SG_PANIC("setup begin: fifo READ ERROR"
		// wanted %x bytes got %x. Stalling out...\n",
		//izeof(*crq), len);
...
void sunxi_udc_dma_completion(struct sunxi_udc *dev, struct sunxi_udc_ep *ep, struct sunxi_udc_request *req)
{
	...
	if (g_dma_debug) {
		DMSG_INFO_UDC("di: (0x%p, %d, %d),(%d,%d)\n", &(req->req), req->req.length, req->req.actual, ep->bEndpointAddress,USB_DIR_IN);
	}
...
static int sunxi_udc_get_frame(struct usb_gadget *_gadget)
{
	...
	DMSG_INFO("sunxi_udc_get_frame is no susport\n");

	//ret = USBC_REG_FRNUM(g_sunxi_udc_io.usb_bsp_hdle);
...
static int sunxi_get_udc_base(struct platform_device *pdev, sunxi_udc_io_t *sunxi_udc_io)
{
...
static int sunxi_get_udc_clock(struct platform_device *pdev, sunxi_udc_io_t *sunxi_udc_io)
{
	struct device_node *np = pdev->dev.of_node;
